...
import { CloseIcon, SitemapIcon, ZoomInIcon, ZoomOutIcon, ResetViewIcon, ArrowDownTrayIcon } from './Icons'; // Import new icons
import { TransformWrapper, TransformComponent, useControls } from 'react-zoom-pan-pinch'; // Import the library components
...
const MermaidModal: React.FC = memo(() => {
  const { isMermaidModalOpen, mermaidCodeForModal, closeMermaidModal } = useModalStore();
  const mermaidContainerRef = useRef<HTMLDivElement>(null);
  const [renderResult, setRenderResult] = useState<{ svg?: string; error?: string }>({});
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    if (!isMermaidModalOpen || !window.mermaid) return;
    ...
    const renderMermaid = async () => {
      try {
        ...
        const { svg } = await window.mermaid.render(diagramId, mermaidCodeForModal);
    ...
    // Delay rendering slightly to allow the modal transition to start
  ...
  }, [isMermaidModalOpen, mermaidCodeForModal]);
  ...
  return (
    ...
    >
      <div
        // --- OLD CODE ---
        // className="aurora-panel p-5 sm:p-6 rounded-lg shadow-2xl w-full sm:max-w-4xl max-h-[90vh] flex flex-col text-gray-200"
        // --- NEW CODE ---
      ...
      >
        ...
        <div 
          // --- OLD CODE ---
          // className="flex-grow overflow-hidden bg-black/20 rounded-md p-4 min-h-[300px] relative mx-5 sm:mx-6"
          // --- NEW CODE ---
        ...
        >
          ...
          {renderResult.svg && (
            <TransformWrapper
              ...
              minScale={0.1}
              maxScale={5}
              limitToBounds={false} // Allows panning outside bounds, snaps back on mouse up
              wheel={{ step: 0.1 }}
              panning={{ disabled: false, velocityDisabled: true }} // Disable velocity for more controlled drag
                ...
                <>
                  ...
                  >
                    <div
                      ...
                      // No classes here, let TransformComponent handle sizing and positioning
                    />
                  </TransformComponent>
                </>
              )}
        ...
        <div 
          // --- OLD CODE ---
          // className="mt-6 flex justify-end flex-shrink-0"
          // --- NEW CODE ---
